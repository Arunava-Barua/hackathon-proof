...
import zkHacker from "./ZkHacker.json";
...
const zkHackerContractAddress = "0x2849CA671e7029BD66Fa119d418a498713927bE7"; // Filecoin calibration

const gainxAbi = gainx.abi;
const gainxTokenAbi = gainxToken.abi;
const redeemTokenAbi = redeemToken.abi;
const zkHackerAbi = zkHacker.abi;
...
*/
  ...
  const [votingListings, setVotingListings] = useState([]);

  const [blacklistListings, setBlacklistListings] = useState([]);
  ...
  const addProjectFn = async () => {
    ...
    try {
      if (window.ethereum) {
        ...
        const txRes = await contract.addProject(receiver, {
          gasLimit: 500000000,
        });
  ...
  */
  ...
  const allExploreListings = async () => {
    let receiver,
      lists = [],
      obj = {};
    try {
      if (window.ethereum) {
        ...
        const txRes = await contract.getExploreProjects();

        txRes.map((element, i) => {
          obj = {
            id: Number(element.id._hex),
            owner: element.owner,
            description: element.description,
            similarityScore: Number(element.similarityScore._hex).toString(),
            upVotes: Number(element.upVotes._hex).toString(),
            downVotes: Number(element.downVotes._hex).toString(),
          };

          lists.push(obj);
        });

        setExploreListings(lists);
  ...
  const allVotingListings = async () => {
    ...
    try {
      if (window.ethereum) {
        ...
        const txRes = await contract.getVotingProjects();
  ...
  const allBlacklistListings = async () => {
    let receiver,
      lists = [],
      obj = {};
    try {
      if (window.ethereum) {
        ...
        const txRes = await contract.getBlacklistProjects();
  ...
  const voteUp = async () => {
    ...
    try {
      if (window.ethereum) {
        ...
        const txRes = await contract.upVoteProject(1, {
          gasLimit: 3000000
        });
  ...
  const voteDown = async () => {
    ...
    try {
      if (window.ethereum) {
        ...
        const txRes = await contract.downVoteProject(1, {
          gasLimit: 3000000
        });
  ...
  useEffect(() => {
    (async () => {
      const res = await allVotingListings();
      console.log("ZK res voting: ", res);
    })();
  }, []);

  useEffect(() => {
    (async () => {
      const res = await allExploreListings();
      console.log("ZK res explore: ", res);
    })();
  }, []);

  useEffect(() => {
    (async () => {
      const res = await allBlacklistListings();
      console.log("ZK res blacklist: ", res);
    })();
  }, []);

  useEffect(() => {
    console.log("ZK listings explore", exploreListings);
    console.log("ZK listings voting", votingListings);
    console.log("ZK listings blacklist", blacklistListings);
  }, []);
